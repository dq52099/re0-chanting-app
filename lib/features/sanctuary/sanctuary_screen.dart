...
class _SanctuaryScreenState extends ConsumerState<SanctuaryScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('圣域 - 管理与个人'),
        actions: [
          IconButton(
            icon: const Icon(Icons.logout, color: Re0Theme.errorRed),
            onPressed: () {
              showDialog(
                context: context,
                builder: (c) => AlertDialog(
                  title: const Text('退出圣域'),
                  ...
                  actions: [
                    ...
                    TextButton(
                      ...
                      child: const Text('退出', style: TextStyle(color: Re0Theme.errorRed))
                    ),
                  ],
                )
              );
